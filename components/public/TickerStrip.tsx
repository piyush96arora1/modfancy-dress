...
export function TickerStrip({ text, speed = 80 }: TickerStripProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative overflow-hidden bg-[#1B2A4A] py-2.5 md:py-3 shadow-sm rounded-b-lg"
    >
      {/* Subtle gradient overlay for depth */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#1B2A4A] via-[#2A3F6A]/30 to-[#1B2A4A] opacity-60 rounded-b-lg" />
      ...
      <div className="absolute left-0 top-0 bottom-0 w-4 md:w-6 bg-gradient-to-r from-[#1B2A4A] via-[#1B2A4A]/70 to-transparent pointer-events-none z-10 rounded-b-lg" />
      <div className="absolute right-0 top-0 bottom-0 w-4 md:w-6 bg-gradient-to-l from-[#1B2A4A] via-[#1B2A4A]/70 to-transparent pointer-events-none z-10 rounded-b-lg" />
    </div>
  )
}
