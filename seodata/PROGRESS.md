...
### 25 Sep 2026: S1 + S2 upload follow-up (Task 19a, `scripts/followup-oct5-batch.ts`)
- ✅ 19 new products (6 women's dandiya lehengas, boys' kediyu, girls' chaniya choli, dandiya sticks, Durga, Mahishasur, adult Ravan, Ravan mask + mukut, Laxman ×2, Hanuman mask, Ram bow, rudraksha mala): description, meta, seo_title and alt text written from the photos; 11 slugs/names fixed (e.g. `ravan-face` → `ravan-mask`, `raavan-fancy-dress` → `ravan-costume-for-adults`, the six lehengas named by colour and print); women's sets sized Adult; categories added (kids, mythology, dandiya/garba for the sticks); 66 image variants generated.
- ✅ Redirects: every upload slug 308s to its new slug; soft-deleted Mahishasur, 3 boys' garba and 5 girls' chaniya choli URLs now point at the new products instead of the category (pending re-uploads 19 → 12).
- ⏳ Owner: dandiya sticks price ₹80 but rent ₹200. Check the women's lehengas really are adult sizes. Uploads from Safari/iOS don't get variants (the browser can't encode WebP), so run `scripts/generate-image-variants.ts --apply` after each batch.
